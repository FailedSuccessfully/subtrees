Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/e34a5a6fcd96e3a449eac94c1d83fc91"
{
	Properties
	{
 ...
 #define PROP_RIMTEX 
			...
			v2f vert(appdata v)
			{
				...
				if (float(0))
				{
					distanceOffset *= min(distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, o.localPos).xyz), float(99));
				}
				float lineWidth = float(0.14);
				#ifdef POI_AUDIOLINK
				if (float(1))
				{
					if (LumaIsAvailable())
			...
			{
				float MainColorFeatherStep = float(0.11) - float(0.5);
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.11) - MainColorFeatherStep) * (shadowMask));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1) == 1)
				{
					...
					#endif
					if (float(1) == float(1)) float(1) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(1), float(1), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(1), float(1), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(1), float(1), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(1) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(1), float(1), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					switch(float(1))
					{
						...
						case 1: rim = lerp(rim, rim * smoothstep(float4(0.4765906,1,0,1).x, float4(0.4765906,1,0,1).y, poiLight.nDotLNormalized), float(0)); break;
					}
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), float(0.35));
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), float(0.35));
				if (float(1))
				{
				...
				poiLight.attenuation = attenuation;
				#endif
				poiLight.directColor = float(1) ? min(float(1.5), _LightColor0.rgb) : _LightColor0.rgb;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
