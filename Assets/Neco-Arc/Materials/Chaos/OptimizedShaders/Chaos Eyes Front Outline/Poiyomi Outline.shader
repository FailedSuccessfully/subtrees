Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/e2d1729528f7bdd478bbde3283b83cd0"
{
	Properties
	{
 ...
 #define POI_VERTEXLIGHT_ON 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(3) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(0.02));
					poiLight.indirectColor = max(poiLight.indirectColor, float(0.02));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.02))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.02))));
				}
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), float(0.6));
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), float(0.6));
				if (float(1))
				{
