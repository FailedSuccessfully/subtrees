Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/df6537777aad61a4d8a3a508d74607af"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			v2f vert(appdata v)
			{
				...
				float lineWidth = (0.125 /*_LineWidth*/);
				#ifdef POI_AUDIOLINK
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if ((3.0 /*_LightingColorMode*/) == 3)
				{
					...
					poiLight.indirectColor = max(poiLight.indirectColor, (0.02 /*_LightingMinLightBrightness*/));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / (0.02 /*_LightingMinLightBrightness*/))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / (0.02 /*_LightingMinLightBrightness*/))));
				}
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), (0.6 /*_LightingMonochromatic*/));
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), (0.6 /*_LightingMonochromatic*/));
